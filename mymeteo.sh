...
printTemp(){
  ...
  check_city=$1
  check_city=$(echo "$check_city" | sed 'y/ąćęłńóśźżĄĆĘŁŃÓŚŹŻ /acelnoszzACELNOSZZ_/')
  city_location=$(getLocation "$check_city" "1")
  ...
  do
    ...
    distance=$(awk '{print ($3-$1)*($3-$1) + ($4-$2)*($4-$2)}' <<< "$city_lat $city_lon $lat $lon")
    min_distance=$(awk '{print ($3-$1)*($3-$1) + ($4-$2)*($4-$2)}' <<< "$city_lat $city_lon $min_lat $min_lon")
